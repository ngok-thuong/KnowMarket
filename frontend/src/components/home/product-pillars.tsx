      ...
      <FadeUp>
        <div className="max-w-2xl">
          ...
          <p className="mt-3 text-paper/55">
            Each flow is designed so smart contracts hold funds and emit canonical events, while the
            app stays fast by reading indexed state from Postgres.
          </p>
        </div>
      </FadeUp>

      <Stagger className="mt-12 grid gap-6 lg:grid-cols-3" staggerDelay={0.15}>
        {pillars.map((p) => (
          <StaggerItem key={p.title} effect="fadeUp">
            <motion.li
              className="group flex h-full flex-col rounded-2xl border border-white/[0.06] bg-ink-muted/60 p-6 transition-colors hover:border-brand/30 hover:bg-ink-soft/80 list-none"
              whileHover={{ y: -4, boxShadow: "0 8px 32px rgba(124,58,237,0.18)" }}
